...
AS $function$
DECLARE
  _co RECORD; _sov_id uuid; _contract_id uuid; _contract_mat text; _biller uuid;
  _mat_resp text; _eq_resp text;
  _mats numeric := 0; _eq numeric := 0; _total numeric := 0; _sort int := 0; _labor numeric := 0;
BEGIN
  SELECT id, project_id, co_number, title, status, pricing_type, org_id, assigned_to_org_id,
         tc_submitted_price,
         co_material_responsible_override, materials_responsible,
         co_equipment_responsible_override, equipment_responsible
    INTO _co FROM public.change_orders WHERE id = _co_id;
  ...
  END IF;
  SELECT from_org_id INTO _biller FROM public.project_contracts WHERE id = _contract_id;
  _biller := COALESCE(_biller, _co.org_id);
  ...
  ELSE
    ...
    SET sov_name = public.co_sov_display_name(_co.co_number, _co.title), contract_id = _contract_id
    WHERE id = _sov_id;
  END IF;

  -- Only the billing company's own (non-private) labor prices; never downstream crew charges.
  WITH lines AS (
    SELECT li.id, li.item_name, li.sort_order,
           COALESCE((SELECT SUM(le.line_total) FROM public.co_labor_entries le
                     WHERE le.co_line_item_id = li.id AND NOT le.is_actual_cost
                       AND le.org_id = _biller), 0) AS amount
  ...
  SELECT COALESCE(SUM(value_amount), 0) INTO _labor FROM public.project_sov_items WHERE sov_id = _sov_id;

  -- Priced via crew-hours toggle (no own labor rows): bill the submitted price.
  IF _labor = 0 AND _biller = _co.org_id AND COALESCE(_co.tc_submitted_price, 0) > 0 THEN
    _labor := GREATEST(_co.tc_submitted_price - _mats - _eq, 0);
    IF _labor > 0 THEN
      INSERT INTO public.project_sov_items
        (project_id, sov_id, sort_order, item_name, item_group, source, value_amount, scheduled_value)
      VALUES (_co.project_id, _sov_id, 1, 'Labor', 'Change order', 'template', _labor, _labor);
    END IF;
  END IF;
...
-- Repair: rebuild every not-yet-invoiced change-order billing schedule.
DO $$
DECLARE r record;
BEGIN
  FOR r IN SELECT s.source_co_id FROM public.project_sov s
           WHERE s.source_co_id IS NOT NULL
             AND NOT EXISTS (SELECT 1 FROM public.invoices i WHERE i.sov_id = s.id)
  LOOP
    PERFORM public.build_co_sov(r.source_co_id);
  END LOOP;
END $$;