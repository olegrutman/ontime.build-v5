...
BEGIN
  ...
  JOIN LATERAL (SELECT round(public.co_grand_total(co.id),2) t) g ON true
  ...
  SELECT _run,'co_not_double_billed','Change order not billed beyond its price',co.project_id,'change_order',co.id,
         round(public.co_grand_total(co.id),2), b.billed, coalesce(co.co_number,'')||' '||coalesce(co.title,'')
  FROM change_orders co
  ...
  WHERE b.billed > round(public.co_grand_total(co.id),2) + 0.01;
  ...
  FROM project_contracts c
  JOIN LATERAL (SELECT round(coalesce(sum(public.co_grand_total(co.id)),0),2) s FROM change_orders co
