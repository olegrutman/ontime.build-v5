...
BEGIN
  ...
  -- Safeguard: the submitting company's approved price is the source of truth.
  IF _biller = _co.org_id AND COALESCE(_co.tc_submitted_price, 0) > 0
     AND ROUND(_labor + _mats + _eq, 2) <> ROUND(_co.tc_submitted_price, 2) THEN
    DELETE FROM public.project_sov_items WHERE sov_id = _sov_id;
    _labor := GREATEST(_co.tc_submitted_price - _mats - _eq, 0);
    IF _labor > 0 THEN
      INSERT INTO public.project_sov_items
        (project_id, sov_id, sort_order, item_name, item_group, source, value_amount, scheduled_value)
      VALUES (_co.project_id, _sov_id, 1, COALESCE(NULLIF(_co.title, ''), 'Labor'), 'Change order', 'template', _labor, _labor);
    END IF;
  END IF;
