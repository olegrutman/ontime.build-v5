...
SELECT id,
    ...
    status,
    pricing_type,
    CASE WHEN co_viewer_role(id) = ANY (ARRAY['gc'::text,'tc'::text]) THEN nte_cap ELSE NULL::numeric END AS nte_cap,
    CASE WHEN co_viewer_role(id) = ANY (ARRAY['gc'::text,'tc'::text]) THEN nte_increase_requested ELSE NULL::numeric END AS nte_increase_requested,
