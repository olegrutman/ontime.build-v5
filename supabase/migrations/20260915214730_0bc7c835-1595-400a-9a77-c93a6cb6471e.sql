...
BEGIN
  ...
  WHERE pi.project_id = _project_id
    ...
    AND (
      ...
      OR (pt.org_id IS NULL AND _user_email IS NOT NULL AND lower(btrim(pi.invited_email)) = _user_email)
      OR (pt.org_id IS NULL AND _user_email IS NOT NULL AND lower(btrim(pt.invited_email)) = _user_email)
      OR (pt.org_id IS NULL AND _org_name IS NOT NULL AND lower(btrim(pt.invited_org_name)) = _org_name)
    )
