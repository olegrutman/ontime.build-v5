...
BEGIN
  ...
  WITH cos AS (
    SELECT
      ...
      -- Crews bill their own logged billable labor at their own rate (private
      -- internal-cost notes are excluded); everyone else bills the price they
      -- submitted upstream.
      CASE WHEN v_org_type = 'FC' THEN
        COALESCE((
          SELECT SUM(le.line_total)
          FROM public.co_labor_entries le
          WHERE le.co_id = co.id
            AND le.org_id = p_from_org_id
