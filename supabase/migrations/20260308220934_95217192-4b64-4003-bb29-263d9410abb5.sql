...
AS $$
BEGIN
  -- Validate caller is a participant OR the work order creator
  IF NOT EXISTS (
    ...
    SELECT 1 FROM change_order_projects co
    WHERE co.id = p_change_order_id
      AND co.created_by = p_user_id
  ) THEN
    RAISE EXCEPTION 'User is not a participant on this work order';
  END IF;

  -- Insert FC labor entry (locked) — only if FC hours > 0
  IF p_fc_hours > 0 THEN
