-- Notify CO creator on approve/reject (restoration after contract-sum trigger refactor)
...
DECLARE
  ...
  _body text;
  _amount numeric;
  _is_tm boolean := COALESCE(NEW.is_tm, false);
  _label text;
  _co_word text;
BEGIN
...
DROP TRIGGER IF EXISTS trg_notify_co_status_change ON public.change_orders;
CREATE TRIGGER trg_notify_co_status_change
AFTER UPDATE OF status ON public.change_orders
FOR EACH ROW
EXECUTE FUNCTION public.notify_co_status_change();