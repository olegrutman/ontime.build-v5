-- Fix notify_project_invite trigger to use actual inviter's org, not project owner's org
...
BEGIN
  ...
  -- Fallback to project owner's org if inviter org not found
  IF _inviter_org IS NULL THEN
    SELECT * INTO _inviter_org FROM organizations WHERE id = _project.organization_id;
  END IF;
  
  -- Create notification when status changes to INVITED
  ...
  IF NEW.invite_status = 'ACCEPTED' AND OLD.invite_status = 'INVITED' THEN
    -- Notify the inviter's organization that the invite was accepted
    INSERT INTO notifications (
      type,
      title,
      body,
