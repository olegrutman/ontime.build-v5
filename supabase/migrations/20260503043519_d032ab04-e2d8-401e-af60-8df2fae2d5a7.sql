-- Fix the action_url in notify_co_status_change: /projects/ → /project/
...
BEGIN
  ...
  INSERT INTO public.notifications (
    ...
    'change_order',
    NEW.id,
    '/project/' || NEW.project_id || '/change-orders/' || NEW.id,
    false
  );
...
-- Drop dead functions that reference stale schema
DROP FUNCTION IF EXISTS public.notify_change_order_status() CASCADE;
DROP FUNCTION IF EXISTS public.cleanup_change_order_notification() CASCADE;
DROP FUNCTION IF EXISTS public.log_change_order_status_change() CASCADE;
DROP FUNCTION IF EXISTS public.create_change_order_checklist() CASCADE;