...
export function useSashaContext(): string {
  ...
  // Individual Change Order detail
  ...
  if (path.startsWith('/project/')) {
    ...
    if (tab === 'team') return context + 'Project Team tab — Shows: list of team members with their organization, role, and permissions. Users can add new team members, manage existing member roles, designate suppliers, and configure material responsibility settings.';
    if (tab === 'schedule') return context + 'Project Schedule tab — Shows: project milestones and timeline with task tracking, due dates, and completion status. Users can add milestones, update progress, and track critical path items.';
    if (tab === 'daily-log') return context + 'Project Daily Log tab — Shows: daily field reports with date, weather conditions, manpower counts by trade, work performed notes, material deliveries, delays/issues, and site photos. Users can create new daily logs, add photos, track manpower, and submit logs for review.';
    if (tab === 'scope') return context + 'Project Scope tab — Shows: contract scope details including included/excluded work categories, scope selections with details, and custom exclusions. Users can review what work is included in the contract scope.';
    if (tab === 'setup') return context + 'Project Setup tab — Shows: step-by-step project configuration wizard with phases: Project Details (name, type, address), Scope (structures, levels, scope of work), Contracts (contract sums, retainage, team relationships), SOV (schedule of values with line items), and Team (invite members). Each phase has a completion indicator.';
    return context + 'Project Overview — Shows: attention banner (items needing action like pending invoices or POs), financial signal cards (contract value, billed, retainage, materials), profit position analysis, operational summary (Change Orders, POs, Invoices counts with status breakdown), team members list, contracts section showing upstream/downstream relationships, budget tracking for labor, readiness checklist (for setup projects), and project scope summary.';
  }
  ...
  if (path === '/settings') return context + 'App Settings page — Shows: application preferences, notification settings, and account configuration options.';
  ...
  if (path === '/financials') return context + 'Financials overview — Shows: financial summary across all projects with total contract values, billed amounts, outstanding balances, receivables and payables breakdown, and profit tracking.';
  ...
  if (path === '/change-orders') return context + 'All Change Orders list — Shows: all Change Orders across projects with status, CO number, title, pricing type, and amounts. Users can filter and search across all projects.';
  if (path.startsWith('/org/team')) return context + 'Organization Team Management — Shows: list of organization members with roles and permissions. Admins can invite new members, update roles, and manage member permissions for creating POs, RFIs, work orders, approving invoices, and viewing financials.';
