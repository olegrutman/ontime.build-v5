  ...
  const calculate = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('projects').select('status, created_by_org_id, organization_id').eq('id', projectId).maybeSingle(),
        ...
        supabase.from('project_sov').select('id, contract_id, is_locked').eq('project_id', projectId).eq('sov_kind', 'base'),
        ...
        supabase.from('supplier_estimates').select('id, supplier_org_id, status').eq('project_id', projectId).is('change_order_id', null),
