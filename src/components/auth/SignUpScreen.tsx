...
interface SignUpScreenProps {
  onSignUp: (email: string, password: string, fullName: string) => Promise<{ error: Error | null; emailSent?: boolean; message?: string }>;
...
export function SignUpScreen({ onSignUp, onGoogleSignIn, onGoToSignIn, onSuccess }: SignUpScreenProps) {
  ...
  // Restore pending signup data (e.g., user verified via email link in a new tab)
  useEffect(() => {
    if (authLoading) return;
    const saved = localStorage.getItem(PENDING_KEY);
    if (saved) {
      try {
        const data = JSON.parse(saved) as { name?: string; email?: string; step?: number };
        if (data.name && !name) setName(data.name);
        if (data.email && !email) setEmail(data.email);
      } catch { /* ignore */ }
    }
    // If user is authenticated already but has no org, skip ahead to role/company step
    if (user && userOrgRoles.length === 0 && step < 3) {
      setStep(3);
    }
  ...
  const handleStep1 = async () => {
    ...
    if (emailSent === false) {
      setFieldErrors({ email: message || 'We could not send a new verification code for this email. Try signing in or use Resend verification.' });
      return;
    }

    // Persist so a callback from the email link can resume the wizard
    try {
      localStorage.setItem(PENDING_KEY, JSON.stringify({ name, email, step: 2 }));
    } catch { /* ignore */ }
  ...
  /* ── Step 2: OTP verify against Supabase ── */
  const handleVerifyOTP = async () => {
    if (otpValue.length < VERIFICATION_CODE_LENGTH) {
      setOtpErrorMsg(`Please enter all ${VERIFICATION_CODE_LENGTH} digits.`);
      setOtpError(true);
      return;
    }
    if (method !== 'email') {
      // Phone path not wired yet
      setOtpErrorMsg('Phone verification is not available yet.');
      setOtpError(true);
      return;
    }
    setLoading(true);
    ...
    setLoading(false);
    if (error) {
      setOtpErrorMsg(error.message || 'Incorrect code. Please try again or request a new one.');
      setOtpError(true);
      return;
    }
    setOtpError(false);
  ...
  const handleFinish = async () => {
    ...
    const orgType = ROLE_TO_ORG_TYPE[selectedRole];
    if (!orgType) {
      setFieldErrors({ form: 'Please choose a role' });
      return;
    }

    setLoading(true);

    // Ensure we have a session — if user landed here via email link in a fresh tab,
    // useAuth should already reflect the session. Double-check.
    ...
    const { error: orgError } = await supabase.rpc('create_organization_and_set_admin', {
      _org_type: orgType,
      _org_name: company.trim(),
      _address: {},
      _org_phone: null as unknown as string,
    ...
    if (orgError) {
      setLoading(false);
      setFieldErrors({ form: orgError.message || 'Failed to create organization' });
      return;
    }
    ...
    // Navigate to dashboard once org exists
