...
export function COStatusActions({
  ...
  const hasAnyAction = canShare || canSendToWIP || canCloseForPricing || canSubmit || canSubmitFCPricing || canRecall || canApprove || canReject || canMarkCompleted || canAcknowledge || canWithdraw || canDelete;
  ...
  return (
    <>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete {co.document_type === 'WO' ? 'work order' : 'change order'} permanently?</AlertDialogTitle>
            <AlertDialogDescription>
              This removes {co.co_number ?? 'this item'} and all of its scope, labor, materials, photos, evidence, and activity history. This cannot be undone.
              {' '}If others have already seen it, consider <strong>Cancel</strong> instead so the record is preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={acting}>Keep it</AlertDialogCancel>
            <AlertDialogAction
              onClick={doDelete}
              disabled={acting}
              className="bg-destructive hover:bg-destructive/90"
