...
const ROLE_COLORS: Record<string, string> = {
  GC: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
  TC: 'bg-teal-100 text-teal-700 dark:bg-teal-900/40 dark:text-teal-300',
  FC: 'bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300',
};
        ...
        {activity.map(entry => (
          <div key={entry.id} className="flex items-start gap-2.5">
            <span className={cn(
              ...
              ROLE_COLORS[entry.actor_role] ?? 'bg-muted text-muted-foreground'
            )}>
              {entry.actor_role}
            </span>
