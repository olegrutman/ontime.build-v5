...
export function COMaterialsPanel({
  ...
  // Strict responsibility: only the party flagged as responsible can manage
  // materials and input pricing. Everyone else sees a read-only view with a
  // banner explaining who owns this section.
  const isResponsibleForMaterials =
    (materialsResponsible === 'GC' && isGC) ||
    (materialsResponsible === 'TC' && (isTC || isFC)) ||
    (!materialsResponsible && (isTC || isGC || isFC));
  ...
  return (
    <div className="co-light-shell">
      ...
      {!isResponsibleForMaterials && responsibleLabel && (
        <div className="px-4 py-2 border-b border-border bg-amber-50/60 text-[11px] text-amber-900 flex items-center gap-1.5">
          <Package className="h-3 w-3 shrink-0" />
          <span>
            <span className="font-semibold">{responsibleLabel}</span> is responsible for materials on this order — items and pricing are entered by {responsibleLabel}.
          </span>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {materials.map(material => {
                  ...
                  const matRowEditable = canManageMaterials && (material as any).org_id === orgId;
                  return (
                    <tr key={material.id} className="border-b border-border last:border-0">
                      ...
                      {(showPricingColumns || draftRows.length > 0) && (
