        ...
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-2 gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <StatusColumn value={changeOrder.status} options={CHANGE_ORDER_STATUS_OPTIONS} size="sm" disabled />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
            {changeOrder.work_type && <span className="capitalize">{changeOrder.work_type.replace('_', ' ')}</span>}
            {changeOrder.location_tag && (
  ...
  const renderOrderSection = (title: string, orders: any[]) => {
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{orders.map(renderWorkOrderCard)}</div>
      </div>
    );
  };
      ...
      {!isProjectNotActive && isBlocked && (
        <Alert className="border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-950/30">
          ...
          <AlertDescription className="text-amber-700 dark:text-amber-300 flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" className="ml-4 border-amber-300 text-amber-700 hover:bg-amber-100 dark:border-amber-700 dark:text-amber-300 dark:hover:bg-amber-900/50"
              onClick={() => {
                const sovTabButton = document.querySelector('[data-value="sov"]') as HTMLButtonElement;
                if (sovTabButton) sovTabButton.click();
              }}
            >
              Go to SOV Tab <ArrowRight className="h-4 w-4 ml-1" />
            </Button>
          </AlertDescription>
        </Alert>
      )}
