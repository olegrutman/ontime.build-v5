...
export function BuildingProfileSection({ projectId, onBuildingTypeChange, onProfileSaved }: BuildingProfileSectionProps) {
  ...
  const selectType = useCallback((pt: ProjectType) => {
    ...
    onBuildingTypeChange?.(pt.slug);
  }, [projectId, onBuildingTypeChange]);

  // Notify parent of building type on mount if already set
  useEffect(() => {
    if (slug) onBuildingTypeChange?.(slug);
  }, [slug]);

  const showUnits = isMultiFamily || slug === 'mixed_use';
  const showBuildings = isMultiFamily || isHotelCommercial;

  // Auto-save when profile is complete enough
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('Error saving building profile:', e);
    }
  };

  // Auto-save on significant changes (debounced via mutation)
  useEffect(() => {
    if (canSave && draft.project_type_id) {
      const timer = setTimeout(() => {
        saveMutation.mutate({ ...draft, is_complete: true });
      }, 1200);
      return () => clearTimeout(timer);
    }
  }, [draft.project_type_id, draft.stories, draft.foundation_types.length, draft.floor_system, draft.roof_system, draft.has_garage]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <Label className="font-semibold text-sm mb-2 block">Project Type</Label>
          ...
          {projectTypes.map(pt => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex flex-col items-center gap-2 p-4 rounded-xl border-2 text-center min-h-[90px] transition-all',
      ...
      {/* Building Structure — only after type selected */}
