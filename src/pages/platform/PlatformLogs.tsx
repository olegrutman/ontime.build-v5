    ...
    >
                ...
                logs.map((log) => (
                  ...
                  >
                    ...
                    <TableCell className="text-sm truncate max-w-[200px]">
                      {log.target_org_name || log.target_user_email || log.target_project_name || '—'}
                    </TableCell>
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selected && (ACTION_TYPE_LABELS[selected.action_type] || selected.action_type)}
            </DialogTitle>
          </DialogHeader>
          {selected && (
            <div className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-3">
                ...
                {selected.target_org_name && (
                  <div>
                    <p className="text-xs text-muted-foreground">Organization</p>
                    <p>{selected.target_org_name}</p>
                  </div>
                )}
                {selected.target_project_name && (
                  <div>
                    <p className="text-xs text-muted-foreground">Project</p>
                    <p>{selected.target_project_name}</p>
                  </div>
                )}
                {selected.target_user_email && (
                  <div>
                    <p className="text-xs text-muted-foreground">User</p>
                    <p>{selected.target_user_email}</p>
                  </div>
                )}
              </div>
              <div>
              ...
              {selected.action_summary && (
                <div>
                  ...
                  <p>{selected.action_summary}</p>
                </div>
              )}
              {selected.before_snapshot && (
                <div>
                  <p className="text-xs text-muted-foreground mb-1">Before Snapshot</p>
                  <pre className="bg-muted/30 rounded p-2 text-xs overflow-x-auto font-mono">
              ...
              {selected.after_snapshot && (
                <div>
                  ...
                  <pre className="bg-muted/30 rounded p-2 text-xs overflow-x-auto font-mono">
                    {JSON.stringify(selected.after_snapshot, null, 2)}
                  </pre>
                </div>
              )}
            </div>
