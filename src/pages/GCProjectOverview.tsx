...
/* ─── Design tokens (same as GCDashboardView) ─── */
const C = {
...
/* ─── Project data ─── */
const project = {
  id: 'CHP', name: '5 Cherry Hills Park', phase: 'Framing · Level 2–4',
  status: 'active', color: '#F5A623', progress: 64, ownerBudget: 420000,
};
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '8px 0', borderBottom: `1px solid ${C.border}`, ...fontLabel }}>
      ...
      {editing ? (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flex: 1 }}>
          ...
          {type === 'textarea' && (
            <button onClick={confirm} style={{ padding: '4px 10px', borderRadius: 6, background: C.amber, color: '#fff', fontSize: '0.68rem', fontWeight: 700, border: 'none', cursor: 'pointer' }}>✓</button>
          )}
...
/* ═══════════════════════════════════════════════════════ */
/*              GC PROJECT OVERVIEW PAGE                   */
/* ═══════════════════════════════════════════════════════ */

export default function GCProjectOverview() {
  const navigate = useNavigate();

  /* ─── Editable contract state ─── */
  const [contract, setContract] = useState({
    contractor: 'Apex Framing Co.',
    value: '368000',
    type: 'Lump Sum',
    signedDate: 'Mar 12, 2025',
    scope: 'Framing L1–4, all labor and equipment',
  });
  ...
  const marginDollar = project.ownerBudget - contractVal;
  const marginPct = contractVal > 0 ? ((marginDollar / project.ownerBudget) * 100).toFixed(1) : '0';
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg }}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        <div style={{ padding: '20px 24px 16px', background: C.surface, borderBottom: `1px solid ${C.border}` }}>
          <div style={{ fontSize: '0.68rem', color: C.muted, marginBottom: 6, ...fontLabel }}>
            <span style={{ cursor: 'pointer' }} onClick={() => navigate('/dashboard')}>Projects</span>
            <span style={{ margin: '0 6px' }}>→</span>
            <span style={{ color: C.ink, fontWeight: 600 }}>{project.name}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: project.color }} />
                <h1 style={{ fontSize: '1.8rem', color: C.ink, margin: 0, ...fontVal }}>{project.name}</h1>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                <span style={{ fontSize: '0.78rem', color: C.muted, ...fontLabel }}>{project.phase}</span>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: C.green, display: 'inline-block', animation: 'pulse 2s infinite' }} />
                <Pill type="pg">Active</Pill>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ padding: '8px 16px', borderRadius: 8, background: C.amber, color: '#fff', fontWeight: 700, fontSize: '0.78rem', border: 'none', cursor: 'pointer', ...fontLabel }}>＋ New CO</button>
              <button style={{ padding: '8px 16px', borderRadius: 8, background: 'transparent', color: C.muted, fontWeight: 600, fontSize: '0.78rem', border: `1px solid ${C.border}`, cursor: 'pointer', ...fontLabel }}>Export PDF</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }} className="max-lg:!grid-cols-2 max-sm:!grid-cols-1">
            ...
            {/* Card 1 — Owner Budget */}
            <KpiCard accent={C.amber} icon="💼" iconBg={C.amberPale} label="OWNER BUDGET" value={fmt(420000)} sub="Full contract value for this project" pills={[{ type: 'pa', text: 'This Project' }]} idx={0}>
            ...
            <KpiCard accent={C.green} icon="🤝" iconBg={C.greenBg} label="TRADE CONTRACTOR CONTRACT (SET BY YOU)" value={fmt(contractVal)} sub={`${contract.contractor} · ${marginPct}% General Contractor margin on this project`} pills={[{ type: 'pg', text: `${fmt(marginDollar)} margin` }, { type: 'pn', text: `${marginPct}%` }]} idx={1}>
              <div style={{ padding: '12px 16px' }}>
                ...
                {dirty && (
                  <button onClick={saveContract} style={{ width: '100%', padding: '10px', borderRadius: 8, background: C.amber, color: '#fff', fontWeight: 700, fontSize: '0.78rem', border: 'none', cursor: 'pointer', marginTop: 12, ...fontLabel }}>
                    Save Contract Changes
                  </button>
                )}
                {dirty && <div style={{ fontSize: '0.6rem', color: C.amber, marginTop: 4, display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <KpiCard accent={C.blue} icon="📝" iconBg={C.blueBg} label="CHANGE ORDERS" value="+$10.2K" sub="3 approved · 0 pending on this project" pills={[{ type: 'pb', text: '3 COs' }]} idx={2}>
              <div style={{ padding: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <KpiCard accent={C.purple} icon="📦" iconBg={C.purpleBg} label="MATERIALS BUDGET" value={fmt(49100)} sub="$66.5K estimated · 74% of estimate ordered" pills={[{ type: 'pg', text: '74% of est' }]} idx={3}>
              <div style={{ padding: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            {/* Card 5 — Open RFIs */}
            <KpiCard accent={C.red} icon="❓" iconBg={C.redBg} label="OPEN RFIs" value="4 Open" sub="8 total · 4 resolved · avg 2.8 days to close" pills={[{ type: 'pr', text: '4 need response' }]} idx={4}>
              <div style={{ padding: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <KpiCard accent={C.green} icon="✅" iconBg={C.greenBg} label="INVOICES PAID" value={fmt(150000)} sub="36% of owner contract · 4 paid invoices" pills={[{ type: 'pg', text: 'On track' }]} idx={5}>
              <div style={{ padding: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <KpiCard accent={C.red} icon="⏳" iconBg={C.redBg} label="PENDING YOUR APPROVAL" value={fmt(18400)} sub="1 invoice awaiting General Contractor sign-off · submitted 4 hr ago" pills={[{ type: 'pr', text: 'Action needed' }]} idx={6}>
              <div style={{ padding: '16px' }}>
                <div style={{ borderRadius: 10, border: `1px solid ${C.border}`, padding: 16, background: C.surface2 }}>
                  ...
                  <div style={{ fontSize: '0.72rem', color: C.muted, marginBottom: 4 }}>Description: Phase 1 labor billing — Trade Co.</div>
            ...
            {/* Card 8 — Change Orders (was Work Orders) */}
              ...
              <div style={{ padding: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    ...
                    <TRow cells={[<TdN>CO-045</TdN>, 'Roof sheathing Bldg A', 'Denver Steel', <TdM>$9,800</TdM>, <Pill type="pw">Review — +$1,800 variance</Pill>]} />
          ...
          {/* ─── Timeline Strip ─── */}
          <div style={{ marginTop: 20, background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, padding: '16px 20px', ...fontLabel }}>
              ...
              {phases.map((p, i) => {
                ...
                return (
                  <div key={i} style={{ position: 'relative', flex: 1 }}>
                    ...
                    {p.active && (
                      <div style={{ position: 'absolute', top: -4, right: '40%', width: 2, height: 40, background: C.amber, borderRadius: 2 }} />
                    )}
                  </div>
                );
              })}
          ...
          {/* ─── Warnings ─── */}
          <div style={{ marginTop: 20, background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden', ...fontLabel }}>
            <div style={{ padding: '14px 16px', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ fontWeight: 700, color: C.ink, fontSize: '0.88rem' }}>🚨 Needs Attention — Cherry Hills</div>
            </div>
