...
describe('pickerReducer', () => {
  ...
  describe('SET_LOCATION', () => {
    it('sets locations on current item', () => {
      const s = act(state(), { type: 'SET_LOCATION', locations: ['L1 · Kitchen', 'L2 · Bath'], locationTypes: ['interior-room', 'interior-room'] });
  ...
  describe('TOGGLE_MULTI_LOCATION', () => {
    ...
    it('trims to single location when toggling off with multiple locations', () => {
      let s = state();
      s = act(s, { type: 'SET_LOCATION', locations: ['A', 'B', 'C'], locationTypes: ['interior-floor', 'interior-floor', 'interior-floor'] });
